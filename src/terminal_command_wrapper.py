...
from src.terminal_stall_detector import (
    ...
    CommandExecution, TerminalStallDetector, get_stall_detector,
    execute_with_stall_detection, format_stall_error
)
...
class TerminalWrapperConfig(BaseModel):
    ...
    default_timeout: Optional[int] = Field(default=300, ge=10, le=3600, description="Default command timeout")
...
class TerminalCommandRequest(BaseModel):
    ...
    shell: Optional[bool] = Field(default=None, description="Execute in shell")
    auto_recover: Optional[bool] = Field(default=None, description="Override auto-recovery setting")
...
class TerminalCommandWrapper:
    ...
    def __init__(self, config: Optional[TerminalWrapperConfig] = None):
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            required_components = [
                "python_version_ok", "subprocess_available", "basic_commands_work", "permissions_ok"
            ]
...
_global_wrapper: Optional[TerminalCommandWrapper] = None


def get_terminal_wrapper(config: Optional[TerminalWrapperConfig] = None) -> TerminalCommandWrapper:
...
def execute_terminal_command_sync(
    ...
    return asyncio.run(execute_terminal_command(
        command=command,
        timeout=timeout,
        cwd=cwd,
        auto_recover=auto_recover,
