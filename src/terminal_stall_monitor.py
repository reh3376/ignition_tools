...
class CommandMonitorRequest(BaseModel):
    ...
    recovery_strategies: list[RecoveryStrategy] = Field(
        default=[
            RecoveryStrategy.INTERRUPT,
            RecoveryStrategy.TERMINATE,
            RecoveryStrategy.KILL,
        ],
...
class TerminalStallMonitor:
    ...
    def __init__(self, config: StallMonitorConfig | None = None):
        ...
        try:
            ...
            try:
                ...
                validation_results["system_resources_ok"] = (
                    memory.available > 256 * 1024 * 1024 and cpu_count >= 1  # 256MB
                )
            except ImportError:
                pass
        ...
        while self.monitoring_active:
            try:
                ...
                for cmd_id, cmd in self.monitored_commands.items():
                    if (
                        cmd.state in [StallState.COMPLETED, StallState.FAILED]
                        and cmd.metrics.start_time < cleanup_threshold
                    ):
...
_global_stall_monitor: TerminalStallMonitor | None = None


def get_stall_monitor(config: StallMonitorConfig | None = None) -> TerminalStallMonitor:
