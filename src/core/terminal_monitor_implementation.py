...
_global_enhanced_monitor: Optional[EnhancedTerminalMonitor] = None


def get_enhanced_terminal_monitor(config: Optional[MonitoringConfig] = None) -> EnhancedTerminalMonitor:
...
def execute_monitored_command_sync(
    ...
    return asyncio.run(execute_monitored_command(
        command=command,
        timeout=timeout,
        cwd=cwd,
        max_retries=max_retries,
