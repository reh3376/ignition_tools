...
def validate_deployment_environment() -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            validation_results["warnings"].append("httpx not available for network checks")
...
class DeploymentManager:
    ...
    async def initialize(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "success": False,
                "error": format_deployment_error(e, "deployment manager initialization")
            }
    ...
    async def get_deployment_status(self) -> Dict[str, Any]:
        ...
        try:
            if not self._is_initialized:
                return {"status": "not_initialized", "error": "Deployment manager not initialized"}
            ...
            try:
                ...
                return {"status": "not_deployed", "message": f"Container {container_name} not found"}
            except Exception as e:
                return {"status": "error", "error": format_deployment_error(e, "status check")}
        
        except Exception as e:
            return {"status": "error", "error": format_deployment_error(e, "deployment status")}
                ...
                if container.status == "running":
                    ...
                    if self.config.health_check.endpoint:
                        try:
                            ...
                            if health_port:
                                start_time = time.time()
                                async with httpx.AsyncClient(timeout=self.config.health_check.timeout) as client:
                                    response = await client.get(
                                        f"http://localhost:{health_port}{self.config.health_check.endpoint}"
                                    )
                                    health_results["response_time"] = time.time() - start_time
                                    health_results["health_check_passed"] = response.status_code == 200
                            else:
                                health_results["errors"].append("No mapped ports found for health check")
                        
                        except ImportError:
                            health_results["errors"].append("httpx not available for health checks")
                        except Exception as e:
                            health_results["errors"].append(f"Health check failed: {e}")
                
                return {
                    "healthy": health_results["container_running"] and health_results["health_check_passed"],
                    "details": health_results
                }
                
            except docker.errors.NotFound:
                return {"healthy": False, "error": f"Container {container_name} not found"}
        
        except Exception as e:
            return {"healthy": False, "error": format_deployment_error(e, "health check")}
...
class Phase126ComprehensiveTester:
    ...
    async def _test_environment_validation(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "message": "Environment ready for deployment" if validation_result["valid"] else "Environment validation failed"
            }
        except Exception as e:
            return {
                "name": "Environment Validation",
    ...
    async def _test_docker_functionality(self) -> Dict[str, Any]:
        ...
        try:
            ...
            container = client.containers.run(
                test_image,
                name="ign-scripts-test-container",
                detach=True,
                remove=True
            )
    ...
    async def _test_deployment_process(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if deployment_result.success and deployment_result.container_id:
                try:
                    container = manager._docker_client.containers.get(deployment_result.container_id)
                    container.stop()
                    container.remove()
                except Exception:
                    pass
    ...
    async def _test_health_checks(self) -> Dict[str, Any]:
        ...
        try:
            ...
            config_valid = isinstance(health_config.endpoint, str) and health_config.interval > 0
            ...
            for workflow_file in workflow_files:
                try:
                    with open(workflow_file, 'r') as f:
                        ...
                        if any(keyword in str(workflow_content).lower() 
    ...
    async def _test_configuration_validation(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "passed": config_validation_works and valid_config.environment == "production",
...
async def run_phase_12_6_implementation() -> Dict[str, Any]:
    ...
    try:
        ...
        print(f"✅ Testing completed: {test_results['passed_tests']}/{test_results['total_tests']} tests passed")
        ...
        implementation_results["message"] = "Phase 12.6 implementation completed successfully"
        ...
        implementation_results["error"] = format_deployment_error(e, "Phase 12.6 implementation")
        implementation_results["end_time"] = datetime.now().isoformat()
        return implementation_results
...
async def deployment_context(config: DeploymentConfig):
    ...
    try:
        init_result = await manager.initialize()
        if not init_result["success"]:
            raise RuntimeError(f"Deployment manager initialization failed: {init_result['error']}")
        yield manager
    finally:
        await manager.cleanup()
