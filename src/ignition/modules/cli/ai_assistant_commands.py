...
@click.option('--validate/--no-validate', default=True, help='Enable/disable knowledge graph validation')
@click.option('--detect-hallucinations/--no-detect-hallucinations', default=True, help='Enable/disable hallucination detection')
@click.option('--output', '-o', type=click.Path(), help='Save analysis results to file')
@click.option('--format', 'output_format', type=click.Choice(['json', 'markdown', 'console']), default='console', help='Output format')
@click.option('--verbose', '-v', is_flag=True, help='Verbose output')
def analyze(file_path: str, validate: bool, detect_hallucinations: bool, output: Optional[str], output_format: str, verbose: bool):
...
@click.option('--file', '-f', 'file_path', type=click.Path(exists=True), help='Python file to analyze')
@click.option('--validate/--no-validate', default=True, help='Enable/disable knowledge graph validation')
@click.option('--verbose', '-v', is_flag=True, help='Verbose output')
def quick_check(code: Optional[str], file_path: Optional[str], validate: bool, verbose: bool):
...
def batch_analyze(directory: str, pattern: str, output_dir: Optional[str], validate: bool, summary_only: bool):
...
def test_connection(neo4j_uri: Optional[str], neo4j_user: Optional[str], neo4j_password: Optional[str]):
    ...
    async def _test_connection():
        ...
        try:
            ...
            console.print(f"[blue]Knowledge graph appears to be accessible[/blue]")
...
def _display_quick_summary(response, verbose: bool):
    ...
    status_color = "green" if response.confidence_score > 0.7 else "yellow" if response.confidence_score > 0.4 else "red"
...
def _display_analysis_json(response, output_file: Optional[str]):
...
def _display_analysis_markdown(response, output_file: Optional[str]):
...
def _display_batch_summary(results, summary_only: bool):
    ...
    summary_table.add_row("High Confidence (>70%)", f"{high_confidence} ({high_confidence/total_files*100:.1f}%)")
    summary_table.add_row("Medium Confidence (40-70%)", f"{medium_confidence} ({medium_confidence/total_files*100:.1f}%)")
    summary_table.add_row("Low Confidence (<40%)", f"{low_confidence} ({low_confidence/total_files*100:.1f}%)")
