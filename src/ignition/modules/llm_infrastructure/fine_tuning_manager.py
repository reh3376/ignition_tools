...
class TrainingDataConfig(BaseModel):
    ...
    max_records: int = Field(default=10000, ge=100, le=100000, description="Maximum training records")
    context_window: int = Field(default=2048, ge=512, le=8192, description="Context window size")
    quality_threshold: float = Field(default=0.8, ge=0.0, le=1.0, description="Quality threshold")
    augmentation_factor: int = Field(default=3, ge=1, le=10, description="Data augmentation factor")
    
    @validator("neo4j_extraction_types")
    def validate_extraction_types(cls, v):
        valid_types = ["Method", "Class", "Function", "Pattern", "CodeFile", "Import", "Parameter"]
        for extraction_type in v:
            if extraction_type not in valid_types:
                raise ValueError(f"Invalid extraction type: {extraction_type}. Valid types: {valid_types}")
        return v
...
class FineTuningConfig(BaseModel):
    ...
    base_model: str = Field(default="llama3.1-8b", description="Base model for fine-tuning")
    ...
    gradient_accumulation_steps: int = Field(default=4, ge=1, le=16, description="Gradient accumulation")
    ...
    save_steps: int = Field(default=500, ge=100, le=2000, description="Save checkpoint steps")
    eval_steps: int = Field(default=250, ge=50, le=1000, description="Evaluation steps")
    max_grad_norm: float = Field(default=1.0, ge=0.1, le=10.0, description="Maximum gradient norm")
...
class FineTuningManager:
    ...
    def __init__(self, training_config: TrainingDataConfig, tuning_config: FineTuningConfig):
    ...
    def _validate_environment(self) -> bool:
        ...
        try:
            ...
            required_vars = [
                ...
                "SME_AGENT_GPU_ENABLED", "SME_AGENT_DEVICE"
            ]
            ...
            try:
                ...
                if not test_client.connect():
                    raise FineTuningValidationError("Cannot connect to Neo4j knowledge graph")
            ...
            if gpu_enabled:
                try:
                    ...
                    raise FineTuningValidationError("PyTorch not available for GPU acceleration")
            else:
    ...
    async def extract_training_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Save dataset
            dataset_path = self.dataset_dir / f"{self.training_config.dataset_name}.jsonl"
            ...
            logger.info(f"✅ Training data extraction complete: {len(augmented_data)} records")
            return result
    ...
    async def _extract_class_data(self) -> List[Dict[str, Any]]:
        ...
        for record in results:
            if record.get('docstring'):
                instruction = f"Describe the {record['class_name']} class and its functionality."
    ...
    async def _extract_function_data(self) -> List[Dict[str, Any]]:
        ...
        for record in results:
            if record.get('docstring'):
                instruction = f"Explain the {record['function_name']} function and how to use it."
    ...
    async def _extract_codefile_data(self) -> List[Dict[str, Any]]:
        ...
        for record in results:
            if record.get('description') or record.get('classes') or record.get('functions'):
    ...
    def _format_method_response(self, record: Dict) -> str:
        ...
        response = f"The {record['method_name']} method in the {record['class_name']} class"
    ...
    async def _augment_training_data(self, training_data: List[Dict]) -> List[Dict]:
        ...
        try:
            ...
            high_quality_data = [
                ...
                if item.get('quality_score', 0.0) >= self.training_config.quality_threshold
            ]
            ...
            # Generate variations
            for item in high_quality_data[:len(high_quality_data)//self.training_config.augmentation_factor]:
                # Create instruction variations
                variations = self._create_instruction_variations(item)
                augmented_data.extend(variations)
            
            logger.info(f"✅ Data augmentation complete: {len(augmented_data)} total records")
    ...
    async def _run_fine_tuning_process(self, training_data: List[Dict], resources: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            phases_result = await self._execute_training_phases(training_data, model_config, training_args)
    ...
    async def _execute_training_phases(self, training_data: List[Dict], model_config: Dict, training_args: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"✅ Phase 1 complete: {len(processed_data)} samples preprocessed")
            ...
            training_metrics = await self._execute_training_loop(processed_data, training_args)
    ...
    async def _execute_training_loop(self, processed_data: List[Dict], training_args: Dict) -> Dict[str, Any]:
        """Execute the training loop (placeholder implementation)."""
        try:
            logger.info("🔄 Executing training loop...")
            
            # Simulate training metrics
            total_steps = len(processed_data) * self.tuning_config.num_epochs // self.tuning_config.batch_size
                    ...
                    if model_dir.is_dir():
                        ...
                        if metadata_file.exists():
                            ...
                            available_models.append({
                                ...
                                "path": str(model_dir),
                                "created": metadata.get("creation_date", "unknown"),
                                "base_model": metadata.get("model_info", {}).get("base_model", "unknown")
                            })
