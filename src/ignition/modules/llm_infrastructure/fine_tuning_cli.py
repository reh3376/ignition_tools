...
@click.option(
    ...
    default=10000,
    type=int,
    help="Maximum number of training records"
)
@click.option(
    "--quality-threshold",
    default=0.8,
...
def extract_data(dataset_name, extraction_types, max_records, quality_threshold, augmentation_factor):
    ...
    try:
        ...
        click.echo(f"   Extraction Types: {', '.join(training_config.neo4j_extraction_types)}")
...
@click.option(
    "--show-datasets",
    is_flag=True,
    help="Show available training datasets"
)
@click.option(
    "--show-models",
    is_flag=True,
    help="Show fine-tuned models"
)
@click.option(
    "--show-config",
    is_flag=True,
...
def status(show_datasets, show_models, show_config):
                ...
                if datasets:
                    for dataset in datasets:
                        ...
                        if metadata_file.exists():
                            ...
                            click.echo(f"      Created: {metadata.get('creation_date', 'Unknown')}")
                        else:
                ...
                if models:
                    for model in models:
                        ...
                        if metadata_file.exists():
                            ...
                            click.echo(f"      Base: {metadata.get('base_model', 'Unknown')}")
                            click.echo(f"      Status: {metadata.get('status', 'Unknown')}")
                            click.echo(f"      Date: {metadata.get('training_date', 'Unknown')}")
                        else:
...
def train(dataset_name, base_model, lora_rank, learning_rate, batch_size, num_epochs, save_steps):
