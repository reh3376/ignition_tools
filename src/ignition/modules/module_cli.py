...
@module_group.command()
@click.argument("name")
@click.option("--template", default="scripting-functions", 
...
def create(name: str, template: str, package: str, scopes: str, description: str):
    ...
    if result:
        console.print(f"✅ Module project created successfully!")
        ...
        console.print(f"  3. Install the .modl file in your Ignition gateway")
    else:
...
def build(project_name: Optional[str], clean: bool, all: bool):
...
def clean(project_name: str):
    ...
    if success:
        console.print(f"✅ Project cleaned successfully")
    else:
        console.print(f"❌ Failed to clean project")


@module_group.command()
@click.argument("project_name")
@click.option("--output", "-o", help="Output directory for packaged module")
def package(project_name: str, output: Optional[str]):
