...
def validate_knowledge_graph_environment() -> Dict[str, Any]:
    ...
    if SENTENCE_TRANSFORMERS_AVAILABLE:
        ...
        validation_result["errors"].append("sentence-transformers package not installed")
    ...
    env_vars = {
        ...
        "KNOWLEDGE_EMBEDDING_MODEL": os.getenv("KNOWLEDGE_EMBEDDING_MODEL", "sentence-transformers/all-MiniLM-L6-v2"),
    }
    ...
    if NEO4J_AVAILABLE and env_vars["NEO4J_URI"] and env_vars["NEO4J_USER"] and env_vars["NEO4J_PASSWORD"]:
        try:
        ...
        validation_result["recommendations"].append("Neo4j connection parameters not configured")
...
class KnowledgeGraphPipeline:
    ...
    async def initialize(self) -> Dict[str, Any]:
        ...
        try:
            ...
            raise SMEAgentValidationError(f"Knowledge graph pipeline initialization failed: {e}")
    ...
    async def extract_knowledge(
        self,
        extraction_types: Optional[List[KnowledgeExtractionType]] = None,
        ...
        try:
            ...
            for extraction_type in extraction_types:
                if extraction_type == KnowledgeExtractionType.ALL:
                    # Extract all types
                    for single_type in [KnowledgeExtractionType.FUNCTIONS, 
    ...
    def _get_extraction_queries(self, extraction_type: KnowledgeExtractionType) -> Dict[str, str]:
        """Get Neo4j queries for different extraction types."""
        queries = {}
        
        if extraction_type == KnowledgeExtractionType.FUNCTIONS:
            queries["system_functions"] = """
            MATCH (f:Function)
    ...
    def _create_knowledge_record(
        ...
        node = neo4j_record["n"] if "n" in neo4j_record else neo4j_record[list(neo4j_record.keys())[0]]
        ...
        title = properties.get("name", properties.get("title", f"{node_type}_{node_id}"))
    ...
    def _generate_content_from_node(self, node: Any, relationships: List[Any]) -> str:
        ...
        if labels:
            content_parts.append(f"This is a {', '.join(labels)} in the Ignition system.")
        ...
        return " ".join(content_parts) if content_parts else f"Node of type {', '.join(labels) if labels else 'Unknown'}"
    ...
    def _generate_context_from_relationships(self, relationships: List[Any]) -> str:
        ...
        for rel_type, rels in rel_groups.items():
            if len(rels) == 1:
                ...
                context_parts.append(f"Has {len(rels)} {rel_type.lower()} relationships")
        
        return "Context: " + ", ".join(context_parts) if context_parts else "No specific context available."
    ...
    def _format_relationship(self, relationship: Any) -> Dict[str, Any]:
        ...
        return {
            ...
            "node_id": str(relationship.get("node", {}).get("id", "unknown")) if relationship.get("node") else "unknown",
            "properties": dict(relationship.get("node", {})) if relationship.get("node") else {},
        }
    ...
    async def _generate_embeddings(self):
        ...
        texts = [f"{record.title} {record.content}" for record in self.extracted_records]
    ...
    async def create_fine_tuning_dataset(
        ...
        if not self.extracted_records:
            raise SMEAgentValidationError("No knowledge records available. Run extract_knowledge first.")
...
async def create_knowledge_dataset(
    ...
    **kwargs
    ...
    if extraction_types:
        extraction_enum_types = [type_map.get(t, KnowledgeExtractionType.ALL) for t in extraction_types]
    else:
