...
class VectorConfig:
    ...
    def __post_init__(self):
        ...
        if self.embedding_model == EmbeddingModel.CUSTOM and not self.custom_model_path:
            raise ValueError("custom_model_path required when embedding_model is CUSTOM")
...
def validate_vector_embedding_environment() -> Dict[str, Any]:
    ...
    if FAISS_AVAILABLE:
        ...
        validation_result["recommendations"].append("faiss-cpu not available - will use slower similarity search")
    ...
    if SENTENCE_TRANSFORMERS_AVAILABLE:
        try:
            ...
            if torch.cuda.is_available():
                ...
                validation_result["recommendations"].append("GPU not available - embeddings will be slower")
        except Exception:
            validation_result["hardware"]["gpu"] = {"available": False}
...
class VectorEmbeddingEnhancement:
    ...
    async def initialize(self) -> Dict[str, Any]:
        ...
        try:
            ...
            raise SMEAgentValidationError(f"Vector embedding initialization failed: {e}")
    ...
    def _initialize_faiss_index(self):
        ...
        if self.config.faiss_index_type == "IndexFlatIP":
            ...
            self.faiss_index = faiss.IndexIVFFlat(quantizer, self.config.embedding_dimension, 100)
        else:
        ...
        try:
            for i, doc in enumerate(documents):
                ...
                if len(embeddings_batch) >= batch_size or i == len(documents) - 1:
                    if FAISS_AVAILABLE and self.faiss_index is not None and embeddings_batch:
                        if NUMPY_AVAILABLE:
    ...
    async def _vector_search(self, query: str, max_results: int) -> List[SearchResult]:
        ...
        for i, (score, idx) in enumerate(zip(scores[0], indices[0])):
            if idx >= 0 and idx < len(doc_ids) and score >= self.config.similarity_threshold:
                doc_id = doc_ids[idx]
            ...
            with self.neo4j_driver.session() as session:
                ...
                for record in result:
                    ...
                    search_result = SearchResult(
                        ...
                        source="graph",
                        metadata={"node_labels": list(node.labels), "properties": properties}
                    )
    ...
    async def _hybrid_search_combined(self, query: str, max_results: int) -> List[SearchResult]:
    ...
    async def _adaptive_search(self, query: str, max_results: int) -> List[SearchResult]:
        """Perform adaptive search based on query characteristics."""
        # Simple heuristics for now - can be enhanced with ML
        if any(keyword in query.lower() for keyword in ["function", "method", "class", "code"]):
            # Code-related queries favor vector search
            return await self._vector_search(query, max_results)
        elif any(keyword in query.lower() for keyword in ["relationship", "connected", "related"]):
    ...
    def get_enhancement_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "index_info": {
                "total_documents": len(self.document_store),
                "index_size": self.faiss_index.ntotal if FAISS_AVAILABLE and self.faiss_index else 0,
