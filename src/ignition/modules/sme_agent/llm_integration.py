...
class LLMConfig:
    ...
    nvidia_gpu_settings: dict[str, Any] = field(
    ...
    macos_unified_settings: dict[str, Any] = field(
        default_factory=lambda: {
    ...
    def __post_init__(self):
        ...
        if self.target_device not in valid_devices:
            raise LLMValidationError(
                f"Invalid target device '{self.target_device}'. Must be one of: {valid_devices}"
            )
        ...
        if self.environment_type not in valid_env_types:
            raise LLMValidationError(
                f"Invalid environment type '{self.environment_type}'. Must be one of: {valid_env_types}"
            )
...
class LLMModelRegistry:
    ...
    @classmethod
    def get_model_info(cls, model_name: str) -> LLMModelInfo | None:
    ...
    try:
        ...
        if TRANSFORMERS_AVAILABLE:
            ...
            if torch.cuda.is_available():
                try:
                    validation_result["system_info"]["cuda_version"] = str(
                        torch.version.cuda
                    )
                except AttributeError:
                ...
                validation_result["warnings"].append(
                    "CUDA not available - LLM will run on CPU (slower)"
                )
        ...
        if DOCKER_AVAILABLE:
            try:
                ...
                client.ping()
                validation_result["system_info"]["docker_version"] = client.version()[
                    "Version"
                ]
            except Exception:
            ...
            validation_result["warnings"].append(
                "Docker not available - containerized deployment disabled"
            )
        ...
        if PSUTIL_AVAILABLE:
            ...
            if memory_gb < 16:
                validation_result["warnings"].append(
                    "Less than 16GB RAM - may limit model performance"
                )
            ...
            if free_space_gb < 50:
                validation_result["warnings"].append(
                    "Less than 50GB free disk space - may not accommodate large models"
                )
        else:
            validation_result["warnings"].append(
        ...
        if validation_result["system_info"].get("cuda_available"):
            validation_result["recommendations"].append(
                "GPU detected - enable GPU acceleration for better performance"
            )
        else:
            validation_result["recommendations"].append(
        ...
        if validation_result["components_available"].get("bitsandbytes"):
            validation_result["recommendations"].append(
                "Use quantization (int8/int4) to reduce memory usage"
            )

        memory_gb = validation_result["system_info"].get("system_memory_gb", 0)
        if memory_gb >= 32:
            validation_result["recommendations"].append(
                "Sufficient RAM for large models without quantization"
            )
        elif memory_gb >= 16:
            validation_result["recommendations"].append(
                "Use quantization for 8B parameter models"
            )
        elif memory_gb > 0:
            validation_result["recommendations"].append(
...
class LLMModelManager:
    ...
    def __init__(self, config: LLMConfig):
        ...
        self.logger.info(
            f"Platform: {self.system_environment.platform} ({self.system_environment.architecture})"
        )
    ...
    def get_model_info(self) -> LLMModelInfo | None:
    ...
    def check_system_requirements(self) -> dict[str, Any]:
        ...
        if self.config.gpu_enabled and cuda_available and gpu_memory_gb:
            ...
            if max_gpu_memory >= model_info.recommended_vram_gb:
                ...
                requirements_check["checks"][
                    "gpu_memory"
                ] = "⚠️ Minimum (consider quantization)"
                requirements_check["recommendations"].append(
        ...
        if model_info.supports_quantization and self.config.quantization != "none":
            requirements_check["checks"][
                "quantization"
            ] = f"✅ {self.config.quantization.upper()}"
    ...
    def generate_response(self, prompt: str, **generation_kwargs) -> dict[str, Any]:
        ...
        if len(prompt) > self.config.max_context * 4:  # Rough token estimate
            raise LLMValidationError(
                f"Prompt too long (estimated tokens exceed {self.config.max_context})"
            )
