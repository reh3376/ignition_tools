...
class SafetyLimit(BaseModel):
    ...
    def validate_limits(cls, v: float | None, info) -> float | None:
        if v is not None and hasattr(info, "data"):
            other_limit = info.data.get(
                "high_limit" if info.field_name == "low_limit" else "low_limit"
            )
            if (
                other_limit is not None
                and info.field_name == "low_limit"
                and v >= other_limit
            ):
...
class EmergencyProcedure(BaseModel):
    ...
    trigger_conditions: list[str] = Field(
        ..., description="Conditions that trigger this procedure"
    )
...
class SafetyConfiguration(BaseModel):
    ...
    watchdog_interval: float = Field(
        default=1.0, gt=0, description="Watchdog check interval"
    )
    emergency_timeout: float = Field(
        default=5.0, gt=0, description="Emergency response timeout"
    )
    ...
    notification_endpoints: list[str] = Field(
        default_factory=list, description="Notification endpoints"
    )
    escalation_enabled: bool = Field(
        default=True, description="Enable alarm escalation"
    )
...
class ProductionSafetySystem:
    ...
    async def initialize(self) -> dict[str, Any]:
        ...
        logger.info(
            "🔧 Step 1-6: Initializing Safety System (crawl_mcp.py methodology)"
        )
    ...
    async def _initialize_emergency_procedures(self) -> None:
        ...
        logger.info(
            f"✅ {len(self.config.emergency_procedures)} emergency procedures initialized"
        )

    async def update_parameter(
        self, parameter_name: str, value: float
    ) -> dict[str, Any]:
    ...
    async def _check_parameter_safety(
        ...
        if violation:
            ...
            await self._create_alarm(
                ...
                value=value,
                limit=(
                    safety_limit.low_limit
                    if violation == "low"
                    else safety_limit.high_limit
                ),
    ...
    async def _should_execute_procedure(
        self, procedure: EmergencyProcedure, reason: str
    ) -> bool:
    ...
    async def _execute_emergency_procedure(self, procedure: EmergencyProcedure) -> None:
        ...
        try:
            ...
            await asyncio.wait_for(
                self._run_procedure_steps(procedure), timeout=procedure.timeout_seconds
            )
    ...
    async def _send_emergency_notifications(self, reason: str) -> None:
        ...
        for endpoint in self.config.notification_endpoints:
            try:
                ...
                logger.critical(
                    f"📢 Emergency notification sent to {endpoint}: {message}"
                )
            except Exception as e:
    ...
    async def _update_safety_state(self) -> None:
        ...
        if any(
            alarm["priority"] in ["CRITICAL", "EMERGENCY"]
    ...
    async def managed_safety_session(self) -> AsyncIterator["ProductionSafetySystem"]:
        ...
        try:
            ...
            if not init_result["success"]:
                raise RuntimeError(
                    f"Safety system initialization failed: {init_result['error']}"
                )
...
async def test_safety_system() -> dict[str, Any]:
    ...
    try:
        ...
        async with safety_system.managed_safety_session():
            ...
            result1 = await safety_system.update_parameter(
                "temperature", 75.0
            )  # Normal
            result2 = await safety_system.update_parameter(
                "temperature", 90.0
            )  # High alarm
