...
class AlarmDefinition(BaseModel):
    ...
    # Escalation settings
    escalation_timeout_minutes: int = Field(default=15, ge=1, description="Escalation timeout")
    auto_clear_enabled: bool = Field(default=False, description="Enable automatic clearing")
    suppression_enabled: bool = Field(default=True, description="Enable alarm suppression")
...
class NotificationRule(BaseModel):
    ...
    alarm_priorities: list[AlarmPriority] = Field(..., description="Alarm priorities to match")
    alarm_categories: list[str] = Field(default_factory=list, description="Alarm categories to match")
    
    # Notification settings
    notification_methods: list[NotificationMethod] = Field(..., description="Notification methods")
    ...
    # Timing settings
    initial_delay_minutes: int = Field(default=0, ge=0, description="Initial notification delay")
    repeat_interval_minutes: int = Field(default=0, ge=0, description="Repeat notification interval")
    max_notifications: int = Field(default=10, ge=1, description="Maximum notifications")
...
class AlarmConfiguration(BaseModel):
    ...
    max_active_alarms: int = Field(default=1000, ge=1, description="Maximum active alarms")
    alarm_retention_days: int = Field(default=90, ge=1, description="Alarm history retention")
    
    # Alarm definitions and rules
    alarm_definitions: list[AlarmDefinition] = Field(..., description="Alarm definitions")
    notification_rules: list[NotificationRule] = Field(..., description="Notification rules")
    
    # System settings
    enable_alarm_suppression: bool = Field(default=True, description="Enable alarm suppression")
...
def format_alarm_error(error: Exception, context: str = "") -> str:
    ...
    if "notification" in error_str or "email" in error_str or "sms" in error_str:
        return f"Notification error in {context}: Check notification service configuration"
...
class ProductionAlarmManager:
    ...
    _notification_handlers: dict[NotificationMethod, Callable] = field(default_factory=dict, init=False)
    ...
    async def initialize(self) -> dict[str, Any]:
        ...
        logger.info("🔧 Step 1-6: Initializing Alarm Manager (crawl_mcp.py methodology)")
    ...
    async def _initialize_notification_handlers(self) -> None:
        ...
        self._notification_handlers[NotificationMethod.EMAIL] = self._send_email_notification
        self._notification_handlers[NotificationMethod.SMS] = self._send_sms_notification
        self._notification_handlers[NotificationMethod.PUSH] = self._send_push_notification
        self._notification_handlers[NotificationMethod.WEBHOOK] = self._send_webhook_notification
        self._notification_handlers[NotificationMethod.VOICE] = self._send_voice_notification
    ...
    async def create_alarm(
        ...
        try:
            ...
            if not definition:
                return {"success": False, "error": f"Alarm definition not found: {alarm_id}"}
            ...
            logger.warning(f"🚨 Alarm created: {definition.name} ({definition.priority.name})")
    ...
    async def suppress_alarm(self, alarm_id: str, user: str) -> dict[str, Any]:
        ...
        try:
            ...
            if not alarm.definition.suppression_enabled:
                return {"success": False, "error": "Alarm suppression not enabled for this alarm"}
    ...
    async def _schedule_notification(self, alarm: AlarmInstance) -> None:
        ...
        for rule in self.config.notification_rules:
            if alarm.priority in rule.alarm_priorities:
                if not rule.alarm_categories or alarm.definition.category in rule.alarm_categories:
    ...
    async def _send_notifications(self, alarm: AlarmInstance, rule: NotificationRule) -> None:
            ...
            for method in rule.notification_methods:
                if method in self._notification_handlers:
                    try:
                        ...
                        logger.error(f"Notification sending error ({method.value}): {e}")
            
        except Exception as e:
            logger.error(f"Notification scheduling error: {e}")
    
    async def _send_email_notification(self, alarm: AlarmInstance, rule: NotificationRule) -> None:
    ...
    async def _send_sms_notification(self, alarm: AlarmInstance, rule: NotificationRule) -> None:
        """Send SMS notification."""
        # In real implementation, this would send actual SMS
        logger.info(f"📱 SMS notification sent for alarm: {alarm.definition.name}")
    
    async def _send_push_notification(self, alarm: AlarmInstance, rule: NotificationRule) -> None:
    ...
    async def _send_webhook_notification(self, alarm: AlarmInstance, rule: NotificationRule) -> None:
    ...
    async def _send_voice_notification(self, alarm: AlarmInstance, rule: NotificationRule) -> None:
            ...
            for rule in self.config.notification_rules:
                if alarm.priority in rule.alarm_priorities and rule.escalation_levels:
                    if alarm.escalation_level <= len(rule.escalation_levels):
                        escalation_config = rule.escalation_levels[alarm.escalation_level - 1]
                        # Process escalation configuration
                        logger.warning(f"⬆️ Alarm escalated: {alarm.definition.name} (Level {alarm.escalation_level})")
    ...
    async def _update_analytics(self) -> None:
        ...
        try:
            ...
            self._alarm_statistics["total_alarms"] = len(self._active_alarms) + len(self._alarm_history)
            self._alarm_statistics["active_alarms"] = len(self._active_alarms)
            self._alarm_statistics["acknowledged_alarms"] = len([a for a in self._active_alarms.values() if a.state == AlarmState.ACKNOWLEDGED])
            ...
            if resolved_alarms:
                ...
                self._alarm_statistics["average_resolution_time"] = sum(resolution_times) / len(resolution_times)
            
            # Calculate escalation rate
            escalated_alarms = len([a for a in self._active_alarms.values() if a.escalation_level > 0])
            total_alarms = len(self._active_alarms)
            self._alarm_statistics["escalation_rate"] = (escalated_alarms / total_alarms * 100) if total_alarms > 0 else 0.0
    ...
    async def _cleanup_old_alarms(self) -> None:
        ...
        self._alarm_history = [
            alarm for alarm in self._alarm_history
    ...
    async def managed_alarm_session(self) -> AsyncIterator["ProductionAlarmManager"]:
        ...
        try:
            ...
            if not init_result["success"]:
                raise RuntimeError(f"Alarm manager initialization failed: {init_result['error']}")
...
async def test_alarm_manager() -> dict[str, Any]:
    ...
    try:
        ...
        async with manager.managed_alarm_session():
            # Test alarm creation
            result1 = await manager.create_alarm("test_alarm_1", source_value=95.0, source_description="High temperature detected")
            if not result1["success"]:
                raise RuntimeError(f"Alarm creation failed: {result1['error']}")
            
            # Test alarm acknowledgment
            result2 = await manager.acknowledge_alarm("test_alarm_1", "test_user", "Investigating high temperature")
        ...
        return {
            ...
            "tests_passed": ["alarm_creation", "alarm_acknowledgment", "alarm_clearing"],
        }
