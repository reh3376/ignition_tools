...
def validate_mpc_environment() -> dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            f"✅ System memory: {memory.available / (1024**3):.1f} GB available"
        )
    except ImportError:
...
class ProcessModel(BaseModel):
    ...
    model_type: str = Field(
        ..., description="Model type (FOPDT, SOPDT, StateSpace, ARX)"
    )
...
class ControlConstraints(BaseModel):
    ...
    y_min: list[float] = Field(
        default_factory=list, description="Minimum output values"
    )
    y_max: list[float] = Field(
        default_factory=list, description="Maximum output values"
    )
...
class MPCConfiguration(BaseModel):
    ...
    optimization_timeout: float = Field(
        default=5.0, gt=0, description="Optimization timeout"
    )
    max_iterations: int = Field(
        default=100, ge=1, description="Maximum solver iterations"
    )
    convergence_tolerance: float = Field(
        default=1e-6, gt=0, description="Convergence tolerance"
    )
    ...
    def validate_horizons(cls, v: int, info) -> int:
        ...
        prediction_horizon = (
            info.data.get("prediction_horizon") if hasattr(info, "data") else None
        )
...
class ProductionMPCController:
    ...
    async def initialize(self) -> dict[str, Any]:
        ...
        logger.info(
            "🔧 Step 1-6: Initializing MPC Controller (crawl_mcp.py methodology)"
        )
    ...
    async def _initialize_state_estimator(self) -> None:
        ...
        logger.info(
            f"✅ State estimation initialized for {self.config.process_model.model_type} model"
        )
    ...
    async def compute_control(
        ...
        try:
            ...
            control_output = await self._solve_mpc_optimization(
    ...
    async def _update_state_estimate(self, measurement: float) -> None:
        ...
        P_pred = (
            A @ self._state_estimator["state_covariance"] @ A.T
    ...
    async def _solve_mpc_optimization(
        ...
        for k in range(self.config.control_horizon):
            ...
            bounds.append(
                (
                    (
                        self.config.constraints.u_min[0]
                        if self.config.constraints.u_min
                        else -np.inf
                    ),
                    (
                        self.config.constraints.u_max[0]
                        if self.config.constraints.u_max
                        else np.inf
                    ),
                )
            )
    ...
    def get_performance_summary(self) -> dict[str, Any]:
        ...
        if len(recent_outputs) == len(recent_setpoints):
            tracking_errors = [
                abs(sp - out)
                for sp, out in zip(recent_setpoints, recent_outputs, strict=False)
            ]
            tracking_stats = {
    ...
    async def managed_control_session(self) -> AsyncIterator["ProductionMPCController"]:
        ...
        try:
            ...
            if not init_result["success"]:
                raise RuntimeError(
                    f"MPC controller initialization failed: {init_result['error']}"
                )
