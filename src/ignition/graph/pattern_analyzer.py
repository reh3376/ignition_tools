...
class PatternAnalyzer:
    ...
    def analyze_function_co_occurrence(self, days_back: int = 30) -> List[Dict[str, Any]]:
        ...
        patterns.sort(key=lambda x: (x["confidence_1_to_2"] + x["confidence_2_to_1"]) / 2, reverse=True)
        ...
        for pattern_type, patterns in analysis_results["patterns"].items():
            for pattern in patterns:
                try:
                    ...
                    self.client.execute_write_query(pattern_query, {
    ...
    def get_recommendations_for_function(self, function_name: str, 
