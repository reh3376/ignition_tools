...
class Neo4jBackupManager:
    ...
    def selective_restore_from_backup(
        ...
        try:
            ...
            current_stats = self._get_database_statistics()
            print(
                f"Current database: {current_stats.get('node_count', 0)} nodes, {current_stats.get('relationship_count', 0)} relationships"  # noqa: E501
            )
            ...
            if success:
                ...
                print(
                    f"Final database: {final_stats.get('node_count', 0)} nodes, {final_stats.get('relationship_count', 0)} relationships"  # noqa: E501
                )
    ...
    if args.action == "backup":
        ...
        if backups:
            ...
            for backup in backups:
                print(
                    f"  • {backup['filename']} - {backup.get('datetime', 'Unknown')} - {backup.get('reason', 'No reason')}"  # noqa: E501
                )
        else:
