...
from ignition.code_intelligence.code_splitter import CodeSplitter, BatchCodeSplitter
...
def detect(directory: str, threshold: int, format: str):
    ...
    try:
        ...
        click.echo(f"❌ Error detecting large files: {str(e)}", err=True)
        sys.exit(1)


@refactor_commands.command()
@click.argument("file_path", type=click.Path(exists=True))
@click.option("--format", "-f", type=click.Choice(["summary", "detailed", "json"]), default="summary", help="Output format")
...
def split(file_path: str, dry_run: bool, no_git: bool):
    ...
    try:
        ...
        click.echo(f"❌ Error during split: {str(e)}", err=True)
        sys.exit(1)
...
def batch_split(directory: str, dry_run: bool, no_git: bool, max_files: int):
    ...
    try:
        ...
        click.echo(f"❌ Error during batch split: {str(e)}", err=True)
        sys.exit(1)
...
def workflow(files: tuple, directory: str, dry_run: bool, no_git: bool):
    ...
    try:
        ...
        click.echo(f"❌ Error during workflow: {str(e)}", err=True)
        sys.exit(1)
    ...
    for file_path in files:
        try:
            ...
            if recommendation:
                ...
                click.echo(f"{str(file_path):<50} {recommendation.physical_lines:<8} {recommendation.complexity_score:<12.1f} {issues:<8}")
            else:
                click.echo(f"{str(file_path):<50} {'N/A':<8} {'N/A':<12} {'N/A':<8}")
        except Exception:
            click.echo(f"{str(file_path):<50} {'ERROR':<8} {'ERROR':<12} {'ERROR':<8}")
        ...
        try:
            ...
            if recommendation:
                data.append({
                    ...
                    "risk_level": "high" if recommendation.complexity_score > 100 else "medium" if recommendation.complexity_score > 50 else "low"
                })
        except Exception:
            data.append({
                "file": str(file_path),
...
@click.option('--format', 'output_format', default='table', type=click.Choice(['table', 'json', 'detailed']), help='Output format')
...
def analyze_branch(source_branch: str, target_branch: str, output_format: str):
    ...
    try:
        ...
        if not analysis:
            click.echo(f"❌ Could not analyze branch differences", err=True)
            return
...
@click.option('--format', 'output_format', default='detailed', type=click.Choice(['table', 'json', 'detailed']), help='Output format')
