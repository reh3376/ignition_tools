...
class CodeIntelligenceDashboard:
    ...
    def get_codebase_health_metrics(self, analysis_period_days: int = 90) -> CodebaseHealthMetrics:
        ...
        try:
            ...
            debt_score, debt_hotspots, refactoring_candidates = self._calculate_technical_debt()
            coupled_files, dependency_depth, circular_deps = self._analyze_dependencies()
            quality_trend, change_freq = self._analyze_quality_trends(analysis_period_days)
            ...
            metrics = CodebaseHealthMetrics(
                ...
                average_file_size=basic_stats.get("total_lines", 0) / max(basic_stats.get("files", 1), 1),
    ...
    def get_dependency_graph(self, max_nodes: int = 50) -> tuple[list[DependencyGraphNode], list[DependencyGraphEdge]]:
        ...
        try:
            ...
            file_data = self.code_manager.client.execute_query(files_query, {"max_nodes": max_nodes})

            nodes = []
            for file_info in file_data:
                # Calculate centrality score
                centrality = (file_info["in_degree"] + file_info["out_degree"]) / max(len(file_data), 1)
            ...
            edge_data = self.code_manager.client.execute_query(edges_query, {"file_paths": file_paths})
    ...
    def _calculate_complexity_distribution(self) -> dict[str, int]:
        ...
        try:
            ...
            complexities = [row["complexity"] for row in self.code_manager.client.execute_query(complexity_query)]
    ...
    def _calculate_maintainability_distribution(self) -> dict[str, int]:
        ...
        try:
            ...
            maintainabilities = [
                row["maintainability"] for row in self.code_manager.client.execute_query(maintainability_query)
            ]

            distribution = {
                "excellent (80-100)": len([m for m in maintainabilities if 80 <= m <= 100]),
    ...
    def _analyze_file_sizes(self) -> tuple[int, list[dict[str, Any]]]:
        ...
        try:
            ...
            large_count = self.code_manager.client.execute_query(large_files_query)[0]["large_count"]
    ...
    def _calculate_technical_debt(
        ...
        try:
            ...
            for file_info in files:
                ...
                maintainability_factor = max(0, (100 - file_info["maintainability"]) / 100.0)
                size_factor = min(file_info["lines"] / 1000.0, 2.0)

                debt_score = (complexity_factor + maintainability_factor + size_factor) / 3.0
    ...
    def _analyze_dependencies(
        ...
        try:
            ...
            dependency_depth = {row["file_path"]: row["max_depth"] or 0 for row in depth_data}
    ...
    def _calculate_file_risk_level(self, complexity: float, lines: int, dependents: int) -> str:
