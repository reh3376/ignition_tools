...
class RefactoringMetrics:
    ...
    developer_satisfaction: Optional[int] = None  # 1-10 scale
...
class RefactoringTracker:
    ...
    def generate_architecture_diagram(self, operation: RefactoringOperation) -> Optional[ArchitectureDiagram]:
...
"""
    ...
    def build_refactoring_impact_report(self, operation_id: Optional[str] = None, days: int = 30) -> Dict[str, Any]:
    ...
    def _generate_operation_recommendations(self, operation: RefactoringOperation) -> List[str]:
        ...
        if complexity_improvement > 10:
            recommendations.append("Excellent complexity reduction achieved. Consider similar patterns for other large files.")
        elif complexity_improvement < 0:
            recommendations.append("Complexity increased. Review the refactoring approach and consider alternative strategies.")
        
        if maintainability_improvement > 10:
            recommendations.append("Significant maintainability improvement. Document the approach for future reference.")
        
        if operation.lines_moved > 500:
            recommendations.append("Large amount of code moved. Ensure comprehensive testing and documentation updates.")
    ...
    def _generate_period_recommendations(self, operations: List[Dict], todos: List[Dict]) -> List[str]:
        ...
        if success_rate < 80:
            recommendations.append(f"Success rate is {success_rate:.1f}%. Review failed operations and improve processes.")
    ...
    def _generate_operation_documentation(self, operation: RefactoringOperation) -> None:
        ...
        for file_path in operation.target_files:
            if Path(file_path).exists():
                self.create_todo_comments(file_path, {
                    'operation_id': operation.operation_id,
                    'split_type': 'responsibility_separation' if operation.operation_type == 'split_file' else 'other'
                })
    ...
    def get_refactoring_statistics(self) -> Dict[str, Any]:
        ...
        avg_complexity_improvement = sum(complexity_improvements) / len(complexity_improvements) if complexity_improvements else 0
