...
class RepositoryAnalyzer:
    ...
    def __init__(self, graph_client, embedder: Optional[SentenceTransformer] = None):
    ...
    def _clone_repository(self, repo_url: str, branch: str) -> Optional[Path]:
    ...
    def _create_directory_node(self, name: str, path: str, repository: str, 
                              file_count: int, subdirectory_count: int, 
                              total_lines: int, root_path: Path) -> None:
        ...
        if path != "":  # Not root directory
            ...
            MERGE (r)-[:CONTAINS]->(d)
            """
            self.client.execute_query(cypher_rel, {
                "repository": repository,
                "path": path,
            })
    ...
    def _create_file_node(self, name: str, path: str, repository: str, directory: str,
                         extension: str, size_bytes: int, lines: int, language: str,
                         file_type: str, content_hash: str, docstring: Optional[str],
                         content: str) -> None:
    ...
    def _create_class_node(self, node: ast.ClassDef, file_path: str, repo_name: str) -> None:
        ...
        properties = [n for n in methods if any(d.id == 'property' for d in n.decorator_list if isinstance(d, ast.Name))]
    ...
    def _create_function_node(self, node: ast.FunctionDef | ast.AsyncFunctionDef, file_path: str, repo_name: str) -> None:
    ...
    def _create_dependency_node(self, dep_spec: str, repo_name: str, dep_type: str, source: str) -> None:
        ...
        dep_name = dep_spec.split('==')[0].split('>=')[0].split('<=')[0].split('>')[0].split('<')[0].split('~=')[0].strip()
        version = None
