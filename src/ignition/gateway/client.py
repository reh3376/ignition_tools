...
class IgnitionGatewayClient:
    ...
    def __init__(self, config: Optional[GatewayConfig] = None, config_name: Optional[str] = None):
    ...
    def _setup_session(self):
        ...
        logger.debug(f"Configured session for {self.config.name}: timeout={self.config.timeout}, verify_ssl={self.config.verify_ssl}")
    ...
    def connect(self) -> bool:
        ...
        """
        try:
            logger.info(f"Connecting to gateway {self.config.name} at {self.config.base_url}")
        ...
        try:
            if self.config.auth_type == "basic":
                ...
                try:
                    ...
                    if not self.config.username or not self.config.password:
                        logger.error("Username and password required for NTLM authentication")
                        return False
                    
                    self.session.auth = HttpNtlmAuth(self.config.username, self.config.password)
                    logger.debug("Configured NTLM authentication")
                except ImportError:
                    logger.error("requests-ntlm package required for NTLM authentication")
                    return False
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                health_result["checks"]["connectivity"]["details"] = f"HTTP {response.status_code}"
            except Exception as e:
            ...
            if self._authenticated or self.connect():
                ...
                health_result["checks"]["authentication"]["details"] = f"Auth type: {self.config.auth_type}"
            else:
                health_result["checks"]["authentication"]["status"] = "unhealthy"
                health_result["checks"]["authentication"]["details"] = "Authentication failed"
                health_result["overall_status"] = "unhealthy"
                return health_result
            ...
            try:
                ...
                gwinfo_response = self.session.get(f"{self.config.base_url}/system/gwinfo", timeout=10)
                if gwinfo_response.status_code == 200:
                    health_result["checks"]["api_access"]["status"] = "healthy"
                    health_result["checks"]["api_access"]["details"] = "Gateway info accessible"
                else:
                    health_result["checks"]["api_access"]["status"] = "warning"
                    health_result["checks"]["api_access"]["details"] = "Limited API access"
            except Exception:
                health_result["checks"]["api_access"]["status"] = "warning"
                health_result["checks"]["api_access"]["details"] = "API access test failed"
            
            # Calculate response time
            response_time_ms = (time.time() - start_time) * 1000
            health_result["checks"]["response_time"]["value_ms"] = round(response_time_ms, 2)
    ...
    def get_gateway_info(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                conn_response = self.session.get(f"{self.config.base_url}/main/web/status/connections", timeout=10)
