...
class Phase16_2TestValidator:
    ...
    def validate_test_environment(self) -> dict[str, any]:
        ...
        results = {"valid": True, "errors": [], "warnings": [], "config_status": {}}
        ...
        if missing_foundation:
            ...
            results["errors"].append(
                f"Missing Phase 16.1 foundation: {missing_foundation}"
            )
            ...
            if not Path(test_dir).exists():
                try:
                    ...
                    results["warnings"].append(
                        f"Created missing test directory: {test_dir}"
                    )
                except Exception as e:
                    results["errors"].append(
                        f"Could not create test directory {test_dir}: {e}"
                    )
                    results["valid"] = False
    ...
    def validate_imports(self) -> dict[str, any]:
        ...
        import_results = {"valid": True, "successful_imports": [], "failed_imports": []}

        # Test imports
        imports_to_test = [
            (
                "BaseSpecializedAgent",
                "src.ignition.modules.sme_agent.specialized.base_specialized_agent",
            ),
            (
                "DistillationWhiskeyAgent",
                "src.ignition.modules.sme_agent.specialized.distillation_whiskey_agent",
            ),
            (
                "PharmaceuticalAgent",
                "src.ignition.modules.sme_agent.specialized.pharmaceutical_agent",
            ),
            (
                "PowerGenerationAgent",
                "src.ignition.modules.sme_agent.specialized.power_generation_agent",
            ),
        ...
        for class_name, module_path in imports_to_test:
            try:
                ...
                import_results["successful_imports"].append(
                    f"{class_name} from {module_path}"
                )
            except ImportError as e:
                import_results["failed_imports"].append(
                    f"{class_name} from {module_path}: {e}"
                )
    ...
    def get_test_summary(self) -> dict[str, any]:
        ...
        return {
            ...
            "recommendations": self._generate_recommendations(
                env_validation, import_validation
            ),
        }

    def _generate_recommendations(
        self, env_results: dict, import_results: dict
    ) -> list[str]:
        ...
        if not env_results["valid"]:
            recommendations.append(
                "Fix environment validation errors before running tests"
            )

        if not import_results["valid"]:
            recommendations.append(
                "Resolve import errors - check Phase 16.2 implementation"
            )
        ...
        if not os.getenv("SPECIALIZED_AGENTS_ENABLED"):
            recommendations.append(
                "Consider setting SPECIALIZED_AGENTS_ENABLED=true for full testing"
            )
...
def get_test_config() -> TestConfiguration:
    ...
    config.test_timeout_seconds = int(
        os.getenv("TEST_TIMEOUT_SECONDS", str(config.test_timeout_seconds))
    )
    config.use_mock_knowledge_base = (
        os.getenv("USE_MOCK_KNOWLEDGE_BASE", "true").lower() == "true"
    )
