...
class StallDetectorTestSuite:
    ...
    def __init__(self, test_config: TestConfig | None = None):
    ...
    async def test_basic_command_execution(self) -> dict[str, Any]:
        ...
        try:
            ...
            if (
                ...
                and len(result.stdout_lines) > 0
                and "Hello, World!" in " ".join(result.stdout_lines)
            ):
                test_result["status"] = "passed"
            else:
                test_result["status"] = "failed"
...
"""
                ...
                result = await execute_with_stall_detection(
                    [sys.executable, str(script_path)], timeout=12, auto_recover=True
                )
    ...
    async def test_recovery_mechanisms(self) -> dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                recovery_tests.append({"recovery_type": "terminate", "error": str(e)})
    ...
    async def test_concurrent_monitoring(self) -> dict[str, Any]:
        ...
        try:
            ...
            commands = [["echo", f"Command {i}"] for i in range(3)]
            ...
            for i, result in enumerate(results):
                if isinstance(result, CommandExecution):
                    ...
                    test_result["details"]["concurrent_test"]["results"].append(
                        {"command_index": i, "error": str(result)}
                    )
