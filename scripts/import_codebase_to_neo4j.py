...
# Type hints for Neo4j (may not be available in all versions)
try:
...
class LocalCodebaseImporter:
    ...
    async def analyze_repository(self, repo_path: Path) -> bool:
        ...
        try:
            ...
            for i, file_path in enumerate(python_files):
                ...
                try:
                    analysis = self._analyze_python_file(
    ...
    async def _clear_repository_data(self, repo_name: str) -> None:
        ...
        try:
            ...
            async with self.driver.session() as session:
                ...
                await session.run(
                    "MATCH (r:Repository {name: $repo_name})-[:CONTAINS]->(f:File)-[:DEFINES]->(c:Class)-[:HAS_METHOD]->(m:Method) DETACH DELETE m",
                    repo_name=repo_name,
                )

                await session.run(
                    "MATCH (r:Repository {name: $repo_name})-[:CONTAINS]->(f:File)-[:DEFINES]->(c:Class)-[:HAS_ATTRIBUTE]->(a:Attribute) DETACH DELETE a",
                    repo_name=repo_name,
                )

                # Delete functions
                await session.run(
                    "MATCH (r:Repository {name: $repo_name})-[:CONTAINS]->(f:File)-[:DEFINES]->(func:Function) DETACH DELETE func",
                    repo_name=repo_name,
                )

                # Delete classes
                await session.run(
                    "MATCH (r:Repository {name: $repo_name})-[:CONTAINS]->(f:File)-[:DEFINES]->(c:Class) DETACH DELETE c",
                    repo_name=repo_name,
                )
    ...
    def _get_importable_module_name(
        self, _file_path: Path, repo_root: Path, relative_path: str
    ) -> str:
    ...
    def _extract_function_parameters(self, func_node) -> list[dict[str, str]]:
    ...
    async def _create_graph(self, repo_name: str, modules_data: list[dict]) -> None:
