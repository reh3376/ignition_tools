...
class LogAnalyzer:
    ...
    def __init__(self):
        self.containers = [
            "ign_scripts_test",
            "ign_scripts_dev",
            "ign_scripts_benchmark",
        ]
        self.log_patterns = {
            "errors": [r"ERROR", r"FAILED", r"Exception", r"Traceback", r"❌"],
            "warnings": [r"WARNING", r"WARN", r"⚠️"],
            "performance": [
                r"duration.*?(\d+\.?\d*)\s*s",
    ...
    def print_summary(self, report: dict[str, Any]) -> None:
        ...
        if any(test_summary.values()):
            ...
            print(
                f"🧪 Test results: {test_summary['passed']} passed, {test_summary['failed']} failed, {test_summary['skipped']} skipped (Total: {total_tests})"
            )
...
def main():
    ...
    parser.add_argument(
        "--report-only",
        action="store_true",
        help="Generate report without printing summary",
    )
