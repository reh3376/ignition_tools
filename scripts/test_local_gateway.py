...
def create_local_gateway_config():
    ...
    for i, config in enumerate(configs, 1):
        print(f"\n{i}. {config['name']}:")
        print(f"   URL: {'https' if config['https'] else 'http'}://{config['host']}:{config['port']}")
...
def get_user_gateway_config():
    ...
    verify_ssl = True
    if use_https:
        verify_ssl = input("Verify SSL certificates? (y/n) [n]: ").strip().lower() in ['y', 'yes', '1', 'true']
...
def test_gateway_connection(config: GatewayConfig):
    ...
    try:
        ...
        if success:
            ...
            print(f"   Response Time: {health['checks']['response_time']['value_ms']}ms")
            
            for check_name, check_result in health['checks'].items():
                status_icon = "✅" if check_result['status'] == 'healthy' else "⚠️" if check_result['status'] == 'warning' else "❌"
                print(f"   {status_icon} {check_name.title()}: {check_result['status']} - {check_result.get('details', '')}")
