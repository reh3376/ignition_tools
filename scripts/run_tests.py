...
def run_performance_tests() -> bool:
    ...
    cmd = [
        "docker", "run", "--rm",
        "-v", f"{Path.cwd()}:/app",
        "-v", f"{Path.cwd()}/test-results:/app/test-results",
        "ign-scripts-test",
        "python", "-m", "pytest", "tests/test_performance.py", "-v",
        "--benchmark-only", "--benchmark-json=/app/test-results/benchmark.json"
    ]
...
def run_ui_tests() -> bool:
    ...
    cmd = [
        ...
        "-v", f"{Path.cwd()}:/app",
        "ign-scripts-test",
        "python", "-m", "pytest", "tests/test_ui.py", "-v", "-m", "ui",
        "--tb=short", "--log-cli-level=INFO"
    ]
...
def run_coverage_tests() -> bool:
    ...
    cmd = [
        "docker", "run", "--rm",
        "-v", f"{Path.cwd()}:/app",
        "-v", f"{Path.cwd()}/coverage-reports:/app/coverage-reports",
        "-v", f"{Path.cwd()}/test-results:/app/test-results",
        "ign-scripts-test",
        "python", "-m", "pytest", "tests/", "-v",
        "--cov=src", "--cov-report=html:/app/coverage-reports/",
...
def run_linting() -> bool:
    ...
    cmd = [
        ...
        "-v", f"{Path.cwd()}:/app",
        "ign-scripts-test",
        "sh", "-c", "cd /app && python -m black --check src tests && python -m isort --check-only src tests && python -m flake8 src tests"
    ]
...
def run_security_scan() -> bool:
    ...
    cmd = [
        ...
        "-v", f"{Path.cwd()}:/app",
        "ign-scripts-test",
        "python", "-m", "bandit", "-r", "src/", "-f", "json", "-o", "/app/test-results/security.json"
    ]
...
def clean_docker_resources() -> None:
    ...
    subprocess.run(["docker", "stop", "ign_scripts_test"], 
                  capture_output=True, stderr=subprocess.DEVNULL)
    subprocess.run(["docker", "stop", "ign_scripts_dev"], 
                  capture_output=True, stderr=subprocess.DEVNULL)
    subprocess.run(["docker", "stop", "ign_scripts_benchmark"], 
                  capture_output=True, stderr=subprocess.DEVNULL)
    
    # Remove containers
    subprocess.run(["docker", "rm", "ign_scripts_test"], 
                  capture_output=True, stderr=subprocess.DEVNULL)
    subprocess.run(["docker", "rm", "ign_scripts_dev"], 
                  capture_output=True, stderr=subprocess.DEVNULL)
    subprocess.run(["docker", "rm", "ign_scripts_benchmark"], 
                  capture_output=True, stderr=subprocess.DEVNULL)
